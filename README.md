<div align="center">

# João Victor Alves de Abreu

### Computer Science student · backend engineering · APIs · systems

<p>
  <a href="https://www.linkedin.com/in/jo%C3%A3ovictoraabreu/">LinkedIn</a>
  &nbsp;·&nbsp;
  <a href="mailto:joaovictoralvesabreu0812@gmail.com">Email</a>
  &nbsp;·&nbsp;
  <a href="https://github.com/JoaoVictorAAbreu-Dev?tab=repositories">Repositories</a>
</p>

<br />

<img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&size=18&duration=2800&pause=900&color=FFFFFF&center=true&vCenter=true&width=560&lines=Building+reliable+backend+systems;Designing+clear+and+secure+APIs;Turning+problems+into+maintainable+software" alt="Typing introduction" />
</div>

<br />

<div align="center">

## About me

</div>

<p align="center">
  Computer Science student from São Paulo, Brazil, focused on backend engineering and system design.<br />
  I build practical software with clear APIs, maintainable architecture and reliable data flows.<br />
  My current work combines Java, Spring Boot, Python, FastAPI, React and TypeScript.
</p>

<br />

<div align="center">

## Current focus

</div>

<p align="center">
  Backend engineering with Java and Spring Boot · REST APIs and authentication ·<br />
  Python and FastAPI · React and TypeScript · PostgreSQL · Docker · testing and CI
</p>

<br />

<div align="center">

## Core stack

<p>
  <img src="https://img.shields.io/badge/Java-000000?style=for-the-badge&logo=openjdk&logoColor=white" alt="Java" />
  <img src="https://img.shields.io/badge/Spring_Boot-000000?style=for-the-badge&logo=springboot&logoColor=white" alt="Spring Boot" />
  <img src="https://img.shields.io/badge/Python-000000?style=for-the-badge&logo=python&logoColor=white" alt="Python" />
  <img src="https://img.shields.io/badge/FastAPI-000000?style=for-the-badge&logo=fastapi&logoColor=white" alt="FastAPI" />
  <img src="https://img.shields.io/badge/React-000000?style=for-the-badge&logo=react&logoColor=white" alt="React" />
  <img src="https://img.shields.io/badge/TypeScript-000000?style=for-the-badge&logo=typescript&logoColor=white" alt="TypeScript" />
  <img src="https://img.shields.io/badge/PostgreSQL-000000?style=for-the-badge&logo=postgresql&logoColor=white" alt="PostgreSQL" />
  <img src="https://img.shields.io/badge/Docker-000000?style=for-the-badge&logo=docker&logoColor=white" alt="Docker" />
</p>

<sub>Also exploring: Rust · Kotlin · Go · C# · C++ · Dart</sub>

</div>

<br />

<div align="center">

## Selected work

</div>

<p align="center">
  <a href="https://github.com/JoaoVictorAAbreu-Dev/fiap-sprint-02motiva-orion-mobile"><strong>Motiva ORION Mobile</strong></a><br />
  Mobile experience created for the Motiva ORION challenge during FIAP Sprint 02.<br />
  <sub>TypeScript</sub>
</p>

<p align="center">──────────</p>

<p align="center">
  <a href="https://github.com/JoaoVictorAAbreu-Dev/Fiap-CP04-edge-computing"><strong>Edge Computing</strong></a><br />
  Distributed vegetation monitoring with connected devices and messaging.<br />
  <sub>Python · IoT · monitoring</sub>
</p>

<p align="center">──────────</p>

<p align="center">
  <a href="https://github.com/JoaoVictorAAbreu-Dev/software-engineering-portfolio"><strong>Software Engineering Portfolio</strong></a><br />
  Portfolio website presenting selected projects and technical case studies.<br />
  <sub>React · TypeScript</sub>
</p>

<p align="center">──────────</p>

<p align="center">
  <a href="https://github.com/JoaoVictorAAbreu-Dev/green-operations-control-center"><strong>Green Operations Control Center</strong></a><br />
  Web platform for roadside vegetation monitoring, prioritization and maintenance planning.<br />
  <sub>Java · Spring Boot · React · PostgreSQL</sub>
</p>

<p align="center">──────────</p>

<p align="center">
  <a href="https://github.com/JoaoVictorAAbreu-Dev/atlas-operations-api"><strong>Atlas Operations API</strong></a><br />
  REST API for operational monitoring, prioritization and field-service planning.<br />
  <sub>Java · Spring Boot · REST</sub>
</p>

<p align="center">──────────</p>

<p align="center">
  <a href="https://github.com/JoaoVictorAAbreu-Dev/taskflow-management-java"><strong>TaskFlow Management</strong></a><br />
  Task-management application focused on organizing work, priorities and execution flow.<br />
  <sub>Java · Spring Boot</sub>
</p>

<p align="center"><a href="https://github.com/JoaoVictorAAbreu-Dev?tab=repositories"><strong>Explore all repositories →</strong></a></p>

<br />

<div align="center">

## GitHub activity

<p align="center">
  <img width="49%" src="https://github-readme-stats.vercel.app/api?username=JoaoVictorAAbreu-Dev&show_icons=true&include_all_commits=true&hide_border=true&theme=dark" alt="GitHub statistics" />
  <img width="49%" src="https://streak-stats.demolab.com/?user=JoaoVictorAAbreu-Dev&hide_border=true&theme=dark" alt="GitHub contribution streak" />
  <img width="49%" src="https://github-readme-stats.vercel.app/api/top-langs/?username=JoaoVictorAAbreu-Dev&layout=compact&hide_border=true&theme=dark" alt="Top languages" />
</p>

<sub>Cards generated from public GitHub activity.</sub>

</div>

<br />

<div align="center">

---

Open to backend and software engineering internship or junior opportunities.

<br /><br />

<strong>Understand the problem. Design the system. Then write the code.</strong>

</div>

<!--
Community projects used:
- https://github.com/DenverCoder1/readme-typing-svg
- https://github.com/devicons/devicon
- https://github.com/anuraghazra/github-readme-stats
- https://github.com/DenverCoder1/github-readme-streak-stats
-->
